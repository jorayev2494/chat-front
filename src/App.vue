        ...
        chatsListen: function (chats) {
            chats.forEach((chat) => {
                ...
                window.Echo.private(`chat.${chat.id}`)
                    .listen(".message.sent", (event) => {
                        window.console.log('Private channel message.sent: ', event);
                        this.$store.commit('chats/pushCurrentMessages', event.message);
                    })
                    .listen('.message.updated', (event) => {
                        window.console.log('Private channel message updated: ', event);
                        this.$store.commit('chats/updateCurrentMessage', event.message);
                    })
                    .listen('.message.deleted', (event) => {
                        window.console.log('Private channel message deleted: ', event);
                        this.$store.commit('chats/deleteCurrentMessage', event.message);
                    });
            });
        },
